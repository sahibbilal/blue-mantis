const blockCards = document.querySelectorAll(
	'.block-card-partners .block-cards'
);
...
const maybeShowMore = () => {
	if (blockCards.length) {
		blockCards.forEach((blockCard) => {
			const partnerCards = blockCard.querySelectorAll(
				'.block-card-partner'
			);

			// If there are more than 8 partner cards, show Load More btn
			if (8 < partnerCards.length) {
