 ...
 *
 * Title:             Card-Speaker
 * Description:       The inner card block for the Cards-Speakers block.
 * Category:          Card
...
<div class="block-card-speaker">
	<div class="block-card-speaker__image-wrapper image-wrapper">
		<?php if ( ! empty( $image ) ) : ?>
			<?php echo wp_kses_post( wp_get_attachment_image( $image, 'person', '', array( 'class' => 'block-card-speaker__image' ) ) ); ?>
		<?php endif; ?>
	</div>
	
	<?php if ( ! empty( $name ) ) : ?>
		<h3 class="block-card-speaker__name"><?php echo wp_kses_post( $name ); ?></h3>
	<?php endif; ?>

	<?php if ( ! empty( $job_title ) || ! empty( $team_or_department ) ) : ?>
		<div class="block-card-speaker__meta">
	...
	<?php if ( ! empty( $linkedin_url ) && ! empty( $name ) ) : ?>
		<a href="<?php echo esc_url( $linkedin_url ); ?>" target="_blank" class="block-card-speaker__linkedin-url" aria-label="<?php echo esc_html( sprintf( __( 'View %s on LinkedIn', 'propel' ), $name ) ); ?>"></a>
	<?php endif; ?>
</div>
