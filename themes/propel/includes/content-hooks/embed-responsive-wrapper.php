...
function oembed_video_wrapper( $cached_html, $url ) {
	...
	if ( strpos( $url, 'wistia' ) !== false ) {
		...
		$class   .= ' vimeo';
		$title    = __( 'vimeo video', 'propel' );
		$response = wp_remote_get( "https://vimeo.com/api/oembed.json?url=$url&width=1680" );
