...
if (instances) {
  instances.forEach(app => {
    ...
    settings.userData = JSON.parse((sessionStorage.getItem('selected')));
	...
	settings.fields = app.getAttribute('data-fields') ? app.getAttribute('data-fields') : '';
	settings.activePage = app.getAttribute('data-activepage') ? app.getAttribute('data-activepage') : '1';
  });
}
